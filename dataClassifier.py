...
def enhancedPacmanFeatures(state, action):
    ...
    if foods:
        features['nearest_food'] = min(util.manhattanDistance(food, pacmanPosition) for food in foods)
    features['nearest_ghost'] = min(util.manhattanDistance(g.getPosition(), pacmanPosition) for g in ghosts)
    features['scare_times'] = sum(g.scaredTimer for g in ghosts)
    features['nearest_ghost_count'] = sum(util.manhattanDistance(g.getPosition(), pacmanPosition) < 3 for g in ghosts)
